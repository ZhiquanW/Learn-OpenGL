...
RenderWindow::RenderWindow(uint32_t w, uint32_t h, std::string name)
    : width{w}, height{h}, name{name}, last_mouse_pos(glm::vec2(0.0f, 0.0f)), mousePosInitialized(true),
    ...
    this->glfwWindow = glfwCreateWindow(this->width, this->height, this->name.c_str(), nullptr, nullptr);
